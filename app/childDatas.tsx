...
export default function ChildDatas() {
  ...
  const handleValidate = () => {
    ...
    sessionStore.setPediatricData({
      ageMode: mode,
      ageValue: parseInt(valeurTemp, 10),
    });

    // Log event
    sessionStore.logEvent("pediatric_data_set", { mode, value: valeurTemp });

    // Save locally purely for UI restoration if user navigates back (optional)
    savedDatas[mode] = valeurTemp;

    router.push("/cprPediatric");
  };
      ...
      <ScrollView contentContainerStyle={styles.Container}>
        ...
        {expanded && (
          <View style={styles.expandedContent}>
            ...
            {monthsInputVisible && (
              <View style={styles.expandedContent}>
                <Text style={styles.expendedButtonText}>
                  {"Entrez l'âge (mois):"}
                </Text>
                <TextInput
                  style={styles.input}
